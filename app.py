...
def sendHeartbeat(service):
    ...
    if service in healthchecks_map:
        requests.get("https://hc-ping.com/{}".format(healthchecks_map[service]), timeout=5)
...
def heartbeat():
    if config.simple_auth.header_key not in request.headers:
        return 'error'
    if request.headers[config.simple_auth.header_key] != config.simple_auth.header_value:
        return 'error'
...
def convert_to_influx(data, tag):
    ...
    ret = data['name'] + ",device=" + tag + \
          " value=" + str(data['value']) + \
          ",kalman=" + str(nv) + " " + str(data['ts'])
    ...
    if tag == 'pg':
        ...
        ret = name + ",device=" + device + " " + value_name + "=" + data['value'] + " " + str(data['ts'])
...
def send_to_narodmon(data, tag):
    ...
    if ts - narodmon_last_send_common < 1 * 60 + 5:  # 1 min
        app.logger.info('[send_to_narodmon]: skip by time for common send (IP 1 min). diff: {} '.format(
    ...
    if ts - narodmon_last_send[tag] < 5 * 60 + 2:  # 5 min
        app.logger.info('[send_to_narodmon]: skip by time for (device 5 min): {}'.format(tag))
        return
...
def send_to_thingspeak(data, token, channel, tag):
    ...
    app.logger.info('[send_to_thingspeak]: channel: ' + str(channel) + ' data: ' + str(data))
    ...
    if tag != 'moxovich' and tag != 'moxovich_second':  # always send moxovich because separate channel
        alive_devices = get_alive_devices(30)
        if not ts_tags[ts_tags_ind] in alive_devices:
            app.logger.info('[send_to_thingspeak]: found dead device: {}, alive: {}'.format(
                ts_tags[ts_tags_ind],
                str(alive_devices)))
            ts_tags_ind = (ts_tags_ind + 1) % len(ts_tags)

        app.logger.info('[send_to_thingspeak]: alive devices: {}'.format(str(alive_devices)))

        if ts - thingspeak_sent_ts < 25:
            app.logger.info('[send_to_thingspeak]: skip by time for: {} (wait for {})'.format(
                str(tag), ts_tags[ts_tags_ind]))

            return 0

        if tag != ts_tags[ts_tags_ind]:
            app.logger.info('[send_to_thingspeak]: skip by order for: {} (wait for {})'.format(
                str(tag), ts_tags[ts_tags_ind]))
            return 0
        ...
        if ts - thingspeak_sent_mox_ts < 25:
            app.logger.info('[send_to_thingspeak] mox: skip by time common for: {}'.format(
                str(tag)))
            return 0
        if ts - mox_ts_send[tag] < 65:  # 1 min
            app.logger.info('[send_to_thingspeak] mox: skip by time per device for: {}'.format(
                str(tag)))
            return 0
        thingspeak_sent_mox_ts = ts
        mox_ts_send[tag] = ts
        app.logger.info('[send_to_thingspeak] mox: beginning to send for {}'.format(str(tag)))
        pass
    # sensors_cache = data
    ...
    try:
        ...
        r = requests.post(
            'http://api.thingspeak.com/channels/' + channel + '/bulk_update.json', json=body)
        resp = r.json()
        if 'success' in resp and resp['success']:
            # sensors_cache = B
            app.logger.info('[send_to_thingspeak]: success: channel:' + str(channel) + ' ' + str(
                resp) + ' for ' + tag + ' body: ' + str(body))
...
def updateDeviceImpl(deviceName, deviceVersion):
    ...
    if deviceName in devsWithTargetVersion:
        ...
        if deviceCurrentVersion < deviceTargetVersion and deviceTargetVersion == binVersion:
...
def update_device():
    ...
    app.logger.info("[deviceUpdate] Requested update for device %s with version %s", deviceName, deviceVersion)
...
def heartbeatHandle():
    ...
    for offDevices in offDevices:
        url = config.common.api_url + config.common.routes.send_data + "?tag=" + offDevices
...
def getInternalSensor(device, sensor, timeout):
    ...
    q = "select time,kalman,value from " + sensor + " where device = '" + device + "' ORDER BY time DESC limit 1"
...
def get_data():
    ...
    app.logger.info("sending to influx")
    headers = {
        'Authorization': config.influxdb.token}
    r = requests.post(
        config.influxdb.url + '/write?db=meteo&precision=s', data="\n".join(influx_data), headers=headers)
    ...
    cnt = 0
    if tag == 'chel':
        cnt = send_to_thingspeak(list(by_ts.values()), config.thingspeak.chel.token,
                                 config.thingspeak.chel.id, 'chel')
    ...
    if tag == 'opus':
        cnt = send_to_thingspeak(list(by_ts.values()), config.thingspeak.opus.token,
                                 config.thingspeak.opus.id, 'opus')

    if tag == 'wave':
        cnt = send_to_thingspeak(list(by_ts.values()), config.thingspeak.wave.token,
                                 config.thingspeak.wave.id, 'wave')

    if tag == 'moxovich':
        cnt = send_to_thingspeak(list(by_ts.values()), config.thingspeak.moxovich.token,
                                 config.thingspeak.moxovich.id, 'moxovich')
    ...
    if tag == 'moxovich_second':
        ...
        cnt = send_to_thingspeak(list(by_ts.values()), config.thingspeak.moxovich_second.token,
                                 config.thingspeak.moxovich_second.id, 'moxovich_second')
        pass
    return jsonify(cnt)
